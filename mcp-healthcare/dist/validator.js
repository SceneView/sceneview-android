 ...
 */
export function validateMedicalCode(code) {
    const issues = [];
    ...
    if (/DICOM|dicom/i.test(code) && !/cornerstone|itk|vtk|dcm4che/i.test(code)) {
        issues.push({
            severity: "info",
            message: "DICOM parsing requires a dedicated library (e.g., dcm4che for JVM, cornerstone.js for web). SceneView handles 3D rendering, not DICOM decoding.",
        });
    }
    if (/\.stl/i.test(code) && !/convert|GLB|glb/.test(code)) {
        issues.push({
            severity: "info",
            message: "STL files must be converted to GLB/glTF for SceneView. Use Blender, meshoptimizer, or gltf-transform for conversion.",
        });
    }
    ...
    if (/alpha|transparency|opacity/i.test(code) && /organ|anatomy|body/i.test(code)) {
        issues.push({
            severity: "info",
            message: "When using transparency for anatomy layers, set material blending mode to TRANSPARENT and render opaque parts first for correct depth sorting.",
        });
    }
