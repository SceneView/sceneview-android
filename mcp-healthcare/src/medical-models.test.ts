...
describe("MEDICAL_MODELS database", () => {
  ...
  it("has BodyParts3D models", () => {
    const bp3d = Object.values(MEDICAL_MODELS).filter((m) =>
      m.source.includes("BodyParts3D")
    );
    expect(bp3d.length).toBeGreaterThanOrEqual(5);
  });

  it("has NIH models", () => {
    const nih = Object.values(MEDICAL_MODELS).filter((m) =>
      m.source.includes("NIH")
    );
    expect(nih.length).toBeGreaterThanOrEqual(4);
  });

  it("has Sketchfab models", () => {
    const sf = Object.values(MEDICAL_MODELS).filter((m) =>
