...
export const DEFAULT_GATEWAY_URL = "https://hub-mcp.mcp-tools-lab.workers.dev/mcp";
/** Public pricing/signup page shown in stubs when no API key is set. */
export const DEFAULT_PRICING_URL = "https://hub-mcp.mcp-tools-lab.workers.dev/pricing";
let nextRpcId = 1;
/**
...
export async function dispatchProxyToolCall(toolName, args, options = {}) {
    ...
    const gatewayUrl = options.gatewayUrl ?? process.env.HUB_MCP_GATEWAY_URL ?? DEFAULT_GATEWAY_URL;
    ...
    try {
        ...
        return {
            content: [
                {
                    type: "text",
                    text: `Failed to reach Hub MCP gateway (${gatewayUrl}): ${detail}.\n\n` +
