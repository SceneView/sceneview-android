...
class LightEstimator {
    ...
    val onUpdated: ((environment: Environment?, mainLight: Light?) -> Unit)? = null
    ...
    fun update(sceneView: SceneView, arFrame: ArFrame) : Pair<Environment?, Light?> {
            ...
            arFrame.frame.lightEstimate.takeIf {
                ...
                when (arFrame.session.lightEstimationMode) {
                    Config.LightEstimationMode.AMBIENT_INTENSITY -> {
                        ...
                        onUpdated?.invoke(environment, mainLight)
                    }
                    Config.LightEstimationMode.ENVIRONMENTAL_HDR -> {
                        ...
                        mainLight = sceneView.mainLight?.clone()?.apply {
                            ...
                            if (environmentalHdrMainLightIntensity) {
                                ...
                                // Filament uses a physically based camera model with lux or lumen
                                // values.
                                // In order to keep the "standard" Filament behavior we scale AR
                                // Core values.
                                // Infos: https://github.com/ThomasGorisse/SceneformMaintained/pull/156#issuecomment-911873565
                                val exposureFactor = sceneView.cameraNode.camera.exposureFactor
                                // Apply the camera exposure factor
                                color *= colorIntensitiesFactors * exposureFactor
                                intensity *= colorIntensity * exposureFactor
                            }
                        }
                        onUpdated?.invoke(environment, mainLight)
                    }
        ...
        return environment to mainLight
    }
