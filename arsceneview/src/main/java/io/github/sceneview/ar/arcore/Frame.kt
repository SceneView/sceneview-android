...
/**
 * Similar to [hitTestFirst], but takes an arbitrary ray in world space coordinates instead of
 ...
 * be normalized.
 * @param plane enable plane results. Default to true
 * @param depth enable depth results. Default to true
 * @param instant enable instant placement results default to Default to true
 *
 * @return an ordered list of intersections with scene geometry, nearest hit first
 */
fun Frame.hitTest(
    ...
    hitTest(origin, origin.size, direction, direction.size).filter { hitResult ->
        hitResult.isValid(
            plane = plane,
            depth = depth,
            instant = instant,
