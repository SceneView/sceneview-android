 ...
 */
interface TrackableComponent<T : Trackable> : PoseComponent {
     ...
     */
    val visibleTrackingStates: Set<TrackingState> get() = setOf(TrackingState.TRACKING)
    var onTrackingStateChanged: ((TrackingState) -> Unit)?
    var onUpdated: ((T) -> Unit)?
     ...
     */
    fun createAnchor() = runCatching { trackable?.createAnchor(pose) }.getOrNull()
     ...
     * @throws java.lang.IllegalStateException if this trackable doesn't support anchors.
     */
    fun createAnchorNode() = createAnchor()?.let { AnchorNode(engine, it) }
}
