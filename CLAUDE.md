...
### Current state (last updated: 2026-04-11 22:00, MCP Gateway #1 LIVE)

- 🟢 **MCP GATEWAY #1 IS LIVE (Stripe production mode)** — from commit `6c938b3d` onward, anyone clicking Subscribe on https://sceneview-mcp.mcp-tools-lab.workers.dev/pricing gets charged a real card. KYC validated instantly (entity shared with existing GitHub Sponsors / Polar Stripe account, auto-entrepreneur existant). 4 products in LIVE catalogue (Pro 19€/190€ + Team 49€/490€) with `price_1TL6...` ids. Webhook `we_1TL7HfEr7tnnFQbdFDu7bmUr` subscribed to 5 events (`checkout.session.completed`, `customer.subscription.{created,updated,deleted}`, `invoice.payment_failed`). Worker version id at go-live: `5947f365-b55b-425c-ab28-f3392caba1c4`. Smoke test 2026-04-11 21:51: all 4 plans return `303 → cs_live_...` (NOT `cs_test_...`), `/health` 200, `/mcp` 401, `/checkout/success?session_id=bogus` 410 (graceful). Cloudflare Secrets rotated TEST → LIVE (`STRIPE_SECRET_KEY` + `STRIPE_WEBHOOK_SECRET`), set via `wrangler secret put` (never committed, never exposed). One restricted key `rk_live_51TKzezEr7tnnFQbdyOTv...` was briefly exposed in a terminal and revoked (expired) via the Stripe dashboard — new `sk_live_...` currently in use was revealed cleanly after the rollback. **Pending: first real paying customer** (needs a human with a real card to click Subscribe and submit — not a tech blocker). Full broadcast in `.claude/NOTICE-2026-04-11-mcp-gateway-live.md`. **Do NOT bump `sceneview-mcp@latest` from 3.6.4 to 4.x until at least one end-to-end real checkout succeeds** — the 3 450 DL/mo existing users would lose local Pro tool access if `@latest` flipped now.
- ⚠️ **CRITICAL BUG FIXED this session (commit `88aec77b`)**: `POST /billing/checkout` was returning 502 `customer_creation can only be used in payment mode` on every anonymous request since the Stripe-first refactor `673ddd88` shipped. **No paying customer could have completed a checkout.** Fix: drop `form.customer_creation = "always"` from `mcp-gateway/src/billing/stripe-client.ts:createCheckoutSession()` — in subscription mode Stripe auto-creates the customer. 168/168 gateway tests pass. **Do NOT re-introduce this flag unconditionally** — if you add a new `mode: "payment"` code path, guard it with `if (mode === "payment")`.
- ✅ **`sceneview-mcp@4.0.0-beta.1` published on npm (commit `73509a95`)** — `@beta` tag only, `@latest` stays on 3.6.4. Lite proxy mode: free tools dispatch locally, Pro tools route to the hosted gateway `/mcp` with Bearer `sv_live_...`. Stderr banner switches between LITE (no api key) and HOSTED (api key present). 17 new tests in `proxy.test.ts`, 2742/2742 total passing. Source: `mcp/src/proxy.ts` (225 lines). Claude Desktop config wired in LITE mode at `~/Library/Application Support/Claude/claude_desktop_config.json`.
- 🔴 **Latest live commits** (`19e55853` → `6c938b3d` → `88aec77b` → `25ce60f9` → `73509a95` → `efd296f1`) — see `.claude/NOTICE-2026-04-11-mcp-gateway-live.md` and top of `.claude/handoff.md` for the full story.
...
- ⚠️ **sceneview-mcp v4.0.0-beta.1 (lite mode) — CODE PERDU / À RÉÉCRIRE**: le commit `31c08302 feat(mcp): v4.0.0-beta.1 lite package with proxy mode` a été créé à 11:49 dans la branche `worktree-agent-ae442902` (ajoutait `mcp/src/proxy.ts`, `mcp/src/proxy.test.ts` avec 10 tests, bumpait package.json 3.6.2 → 4.0.0-beta.1, dispatchProxyToolCall forward des Pro tools vers le hosted /mcp, banner stderr lite mode). Le commit est ensuite **tombé hors de l'historique visible** de sa branche (probablement reset/rebase quand le pivot Stripe-first 673ddd88 a rendu obsolète l'architecture magic-link originale). Vérifié 2026-04-11 15:45 : `mcp/src/proxy.ts` et `proxy.test.ts` n'existent plus nulle part dans le repo, `mcp/package.json` reste à 3.6.4. Le commit vit toujours dans `git reflog refs/heads/worktree-agent-ae442902@{3}` et peut être récupéré par SHA si besoin. **NE PAS publier en l'état** — cherry-pick orphelin après 4h+ de pivot Stripe-first = rollback implicite. Session dédiée requise : réécrire le proxy layer compatible avec le flux actuel (API key provisionnée via webhook Stripe + KV handoff single-use, plus de magic-link/dashboard user), puis publish `npm publish --tag beta`. Le Worker à `sceneview-mcp.mcp-tools-lab.workers.dev` est prêt à recevoir les requêtes proxied.
