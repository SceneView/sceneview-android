...
- ✅ **Telemetry Worker** (session `flamboyant-neumann`, PR #815): `telemetry-worker/` Cloudflare Worker (Hono + D1 + KV) ingesting anonymous events from `mcp/src/telemetry.ts`. 6 endpoints (events, batch, stats, timeseries, export, health), client-side batching in `mcp/src/telemetry.ts`, dashboard.html with CSV export, `X-RateLimit-*` headers, CI workflow. **79 tests** (54 worker + 25 mcp). Opus review passed. NOT deployed — Thomas needs D1/KV create + `wrangler deploy` + CNAME `telemetry.sceneview.io`.
