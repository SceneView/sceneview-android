...
describe("FURNITURE_MODELS database", () => {
  ...
  it("has Poly Haven models", () => {
    const ph = Object.values(FURNITURE_MODELS).filter((m) =>
      m.source.includes("Poly Haven")
    );
    expect(ph.length).toBeGreaterThanOrEqual(5);
  });

  it("has Khronos models", () => {
    const khr = Object.values(FURNITURE_MODELS).filter((m) =>
  ...
  it("has Sketchfab models", () => {
    const sf = Object.values(FURNITURE_MODELS).filter((m) =>
