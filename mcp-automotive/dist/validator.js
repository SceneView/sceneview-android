 ...
 */
export function validateAutomotiveCode(code) {
    const issues = [];
    ...
    if (/\.fbx/i.test(code) && !/convert|GLB|glb/.test(code)) {
        issues.push({
            severity: "info",
            message: "FBX files must be converted to GLB/glTF for SceneView. Use Blender (File > Export > glTF) or FBX2glTF tool.",
        });
    }
    if (/\.blend/i.test(code)) {
        issues.push({
            severity: "error",
            message: ".blend files cannot be loaded directly by SceneView. Export to GLB from Blender (File > Export > glTF 2.0).",
        });
    }
    if (/KHR_materials_variants/i.test(code) || /material.?variant/i.test(code)) {
        issues.push({
            severity: "info",
            message: "KHR_materials_variants is ideal for car configurators (paint colors, interior trims). Ensure your GLB contains variant data exported from Blender or 3ds Max.",
        });
    }
    if (/onFrame.*rotation.*\+=/i.test(code) || /rotationAngle\s*\+=/i.test(code)) {
        issues.push({
            severity: "info",
            message: "Turntable rotation detected. For smooth 60fps rotation, keep the increment small (0.1-0.5 degrees per frame). Consider using Compose animation APIs for frame-rate-independent rotation.",
        });
    }
