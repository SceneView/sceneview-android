...
import { CAR_MODELS, CAR_MODEL_IDS, listCarModels, getCarModel, formatCarModelList, } from "./car-models.js";
describe("CAR_MODELS database", () => {
    ...
    it("has Sketchfab models", () => {
        const sf = Object.values(CAR_MODELS).filter((m) => m.source.includes("Sketchfab"));
        expect(sf.length).toBeGreaterThanOrEqual(5);
    });
    it("has engine models", () => {
        const engines = Object.values(CAR_MODELS).filter((m) => m.category === "engine");
        expect(engines.length).toBeGreaterThanOrEqual(1);
    });
    it("has interior models", () => {
        const interiors = Object.values(CAR_MODELS).filter((m) => m.category === "interior");
        expect(interiors.length).toBeGreaterThanOrEqual(1);
    });
    it("has complete car models", () => {
        const cars = Object.values(CAR_MODELS).filter((m) => m.category === "complete-car");
