...
function getCached(apiKey: string): SubscriptionStatus | undefined {
...
function setCache(apiKey: string, status: SubscriptionStatus): void {
...
function checkDevAllowlist(apiKey: string): SubscriptionStatus {
 ...
 */
export async function validateApiKey(apiKey: string): Promise<SubscriptionStatus> {
  // Check cache first
 ...
 */
export async function recordUsage(apiKey: string, toolName: string): Promise<void> {
  const timestamp = new Date().toISOString();
  process.stderr.write(
    `[billing] ${timestamp} usage: key=${apiKey.slice(0, 8)}… tool=${toolName}\n`,
  );
}
