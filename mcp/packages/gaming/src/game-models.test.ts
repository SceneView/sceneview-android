...
describe("GAME_MODELS database", () => {
  ...
  it("has Khronos sample models", () => {
    const khronos = Object.values(GAME_MODELS).filter((m) =>
      m.source.includes("Khronos")
    );
    expect(khronos.length).toBeGreaterThanOrEqual(4);
  });

  it("has Kenney models", () => {
    const kenney = Object.values(GAME_MODELS).filter((m) =>
      m.source.includes("Kenney")
    );
    expect(kenney.length).toBeGreaterThanOrEqual(4);
  });

  it("has Sketchfab models", () => {
    const sf = Object.values(GAME_MODELS).filter((m) =>
  ...
  it("has Quaternius models", () => {
    const q = Object.values(GAME_MODELS).filter((m) =>
      m.source.includes("Quaternius")
    );
