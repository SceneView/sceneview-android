...
export interface WebViewerEmbedOptions {
  /** URL of a .rrd recording to load at startup. Absolute or relative. */
  rrdUrl: string;
  /** CSS id of the container div the viewer mounts into. Default: "rerun-viewer" */
  containerId?: string;
  /** Hide the Rerun welcome screen. Default: true */
  hideWelcome?: boolean;
  /** Rerun Web Viewer version to load from jsdelivr. Default: "latest" */
  viewerVersion?: string;
  /** Optional [startNanos, endNanos] time range to jump to on load. */
  timeRange?: [number, number];
}

export interface WebViewerEmbedResult {
  html: string;
  script: string;
  fullDocument: string;
}
 ...
 */
export function generateWebViewerEmbed(
  options: WebViewerEmbedOptions,
): WebViewerEmbedResult {
  const {
    rrdUrl,
    containerId = "rerun-viewer",
    hideWelcome = true,
  ...
  if (!/^[a-zA-Z0-9_-]+$/.test(containerId)) {
    throw new Error(
      `containerId "${containerId}" must contain only letters, digits, "_" or "-"`,
    );
  }
